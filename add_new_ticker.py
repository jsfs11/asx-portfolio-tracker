...
class ASXTickerIntegrator:
    ...
    def add_to_franking_database(self, ticker: str, sector: str, franking_rate: int):
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?)
            ''', (ticker, franking_rate, sector, 'estimated', datetime.now().isoformat()))
            
            conn.commit()
            conn.close()
            print(f"✅ Added {ticker} to franking database: {franking_rate}% franked, {sector} sector")
    ...
    def fetch_current_price(self, ticker: str) -> Optional[float]:
        ...
        try:
            # Try EODHD API first
            price = self.tracker.get_current_price_eodhd(ticker, EODHD_API_KEY, force=False)
            ...
            if price is not None:
                ...
                if fallback_price is not None:
                    ...
                    print(f"⚠️  Using fallback price for {ticker}: ${fallback_price:.3f}")
    ...
    def validate_ticker(self, ticker: str) -> bool:
        ...
        try:
            ...
            cursor.execute("SELECT COUNT(*) FROM transactions WHERE stock = ?", (ticker,))
            if cursor.fetchone()[0] > 0:
                print(f"⚠️  {ticker} already exists in portfolio")
                return True  # Exists but that's okay
            
            # Check franking database
            cursor.execute("SELECT COUNT(*) FROM franking_static_data WHERE stock = ?", (ticker,))
...
def main():
    parser = argparse.ArgumentParser(description='Add new ASX ticker with automated data population')
