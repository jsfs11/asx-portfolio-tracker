...
def calculate_stock_contributions():
    ...
    attribution_df['contribution_to_return'] = (attribution_df['unrealized_pnl'] / attribution_df['total_cost'].sum()) * 100
    
    # Sort by contribution
    attribution_df = attribution_df.sort_values('contribution_to_return', ascending=False)
...
def create_attribution_scatter(attribution_df):
    ...
    # Add quadrant lines
    fig.add_hline(y=0, line_dash="dash", line_color="gray", opacity=0.5)
    fig.add_vline(x=attribution_df['weight'].mean(), line_dash="dash", line_color="gray", opacity=0.5)
...
def generate_attribution_report():
    ...
    print(f"Weighted Average Return: {(attribution_df['contribution_to_return']).sum():.2f}%")
    ...
    for _, row in attribution_df.iterrows():
        print(f"{row['stock']:<6} {row['weight']:<8.2f}% {row['return_pct']:<8.2f}% "
    ...
    waterfall_filename = f"attribution_waterfall_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
    ...
    treemap_filename = f"attribution_treemap_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
    ...
    scatter_filename = f"attribution_scatter_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
    ...
    if len(winners) > 0:
        print(f"• {len(winners)} positions are profitable (avg return: {winners['return_pct'].mean():.2f}%)")
    if len(losers) > 0:
        print(f"• {len(losers)} positions are losing (avg return: {losers['return_pct'].mean():.2f}%)")
    ...
    if len(positive_contributors) > 0:
        print(f"• {len(positive_contributors)} stocks contributed positively (+{positive_contributors['contribution_to_return'].sum():.3f}%)")
    if len(negative_contributors) > 0:
        print(f"• {len(negative_contributors)} stocks contributed negatively ({negative_contributors['contribution_to_return'].sum():.3f}%)")
