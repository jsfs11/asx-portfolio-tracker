...
def create_portfolio_overview_chart(summary):
    ...
    fig.update_layout(title="Portfolio Allocation", showlegend=True, height=400)
...
def create_performance_bar_chart(summary):
    ...
    for stock, position in summary["positions"].items():
        ...
        pnl_pct = (
            ((position.current_price / position.avg_cost - 1) * 100)
...
def create_positions_table(summary):
    ...
    for stock, position in summary["positions"].items():
        ...
        effective_yield = dividend_yield * (
        ...
        if (
            hasattr(st.session_state.tracker, "franking_calculator")
            and st.session_state.tracker.franking_calculator
        ):
            row_data.update(
                {
...
def add_transaction(stock, action, quantity, price, transaction_date):
    ...
    try:
        ...
        return (
            True,
            f"Transaction added: {action.title()} {quantity} shares of {stock} at ${price:.2f}",
        )
    except Exception as e:
...
def update_prices(force=False):
    ...
    try:
        summary = st.session_state.tracker.get_portfolio_summary(
            EODHD_API_KEY, True, force
        )
...
def create_franking_credits_chart(franking_summary):
    ...
    sorted_data = sorted(
        zip(stocks, franking_credits, franking_rates, sectors, market_values),
        key=lambda x: x[1],
        reverse=True,
    )
...
def create_franking_by_sector_chart(franking_summary):
    ...
    fig.update_layout(title="Franking Credits by Sector", height=400)
