...
class DividendTracker:
    ...
    def get_stock_dividends(
        self, stock: str, from_date: Optional[str] = None
    ) -> List[Dividend]:
        ...
        return [
            Dividend(stock=row[0], ex_date=row[1], amount=row[2], currency=row[3])
            for row in results
        ]

    def calculate_dividend_yield(
        self, stock: str, current_price: float, period_months: int = 12
    ) -> float:
        """Calculate dividend yield over specified period"""
        from_date = (datetime.now() - timedelta(days=period_months * 30)).strftime(
            "%Y-%m-%d"
        )
    ...
    def calculate_portfolio_dividends(self, positions: Dict) -> Dict:
        ...
        for stock, position in positions.items():
            ...
            annual_dividend_estimate = (
                (annual_yield / 100) * position.current_price * position.quantity
            )
    ...
    def record_dividend_payment(
        ...
        print(
            f"Recorded dividend payment: {stock} {shares_held} shares × ${dividend_per_share:.4f} = ${total_payment:.2f}"
        )
    ...
    def fetch_dividend_data_eodhd(
        self, stock: str, api_key: str = "demo"
    ) -> List[Dividend]:
        ...
        try:
            ...
            params = {
                "api_token": api_key,
                "fmt": "json",
                "from": (datetime.now() - timedelta(days=365 * 2)).strftime(
                    "%Y-%m-%d"
                ),  # 2 years
            }
            ...
            for div_data in data:
                ...
                # Store in database
                self.add_dividend(
                    stock=stock, ex_date=dividend.ex_date, amount=dividend.amount
                )
    ...
    for stock, dividends in ASX_DIVIDEND_DATA.items():
        for div in dividends:
            tracker.add_dividend(
                ...
                amount=(
                    float(div["amount"])
                    if isinstance(div["amount"], (int, float, str))
                    else 0.0
                ),
            )
...
if __name__ == "__main__":
    ...
    cba_yield = tracker.calculate_dividend_yield(
        "CBA", 180.0
    )  # Assume $180 current price
