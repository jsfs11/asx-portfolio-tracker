...
class StaticFrankingDatabase:
    ...
    def __init__(self):
        self.franking_data = {
            ...
            'CBA': {'franking_rate': 100, 'sector': 'Financials', 'reliability': 'high'},
            'WBC': {'franking_rate': 100, 'sector': 'Financials', 'reliability': 'high'},
            'ANZ': {'franking_rate': 100, 'sector': 'Financials', 'reliability': 'high'},
            'NAB': {'franking_rate': 100, 'sector': 'Financials', 'reliability': 'high'},
            ...
            'WOW': {'franking_rate': 100, 'sector': 'Consumer Staples', 'reliability': 'high'},
            'COL': {'franking_rate': 100, 'sector': 'Consumer Staples', 'reliability': 'high'},
            'WES': {'franking_rate': 100, 'sector': 'Consumer Staples', 'reliability': 'high'},
            
            # Telecommunications - Usually 100% franked
            'TLS': {'franking_rate': 100, 'sector': 'Telecommunications', 'reliability': 'high'},
            'TPG': {'franking_rate': 100, 'sector': 'Telecommunications', 'reliability': 'medium'},
            ...
            'APA': {'franking_rate': 100, 'sector': 'Utilities', 'reliability': 'medium'},
            
            # Insurance - Usually 100% franked
            'IAG': {'franking_rate': 100, 'sector': 'Financials', 'reliability': 'high'},
            'SUN': {'franking_rate': 100, 'sector': 'Financials', 'reliability': 'high'},
            'QBE': {'franking_rate': 100, 'sector': 'Financials', 'reliability': 'high'},
            ...
            'NXT': {'franking_rate': 0, 'sector': 'Technology', 'reliability': 'medium'},
            
            # Healthcare - Mixed
            'COH': {'franking_rate': 100, 'sector': 'Healthcare', 'reliability': 'high'},
            'RHC': {'franking_rate': 100, 'sector': 'Healthcare', 'reliability': 'medium'},
            'SHL': {'franking_rate': 100, 'sector': 'Healthcare', 'reliability': 'medium'},
            
            # Industrial - Usually 100% franked
            'TCL': {'franking_rate': 100, 'sector': 'Industrials', 'reliability': 'high'},
            'ALL': {'franking_rate': 100, 'sector': 'Industrials', 'reliability': 'high'},
            'BXB': {'franking_rate': 100, 'sector': 'Industrials', 'reliability': 'medium'},
            ...
            'JBH': {'franking_rate': 100, 'sector': 'Consumer Discretionary', 'reliability': 'high'},
            'HVN': {'franking_rate': 100, 'sector': 'Consumer Discretionary', 'reliability': 'high'},
            'SUL': {'franking_rate': 100, 'sector': 'Consumer Discretionary', 'reliability': 'medium'},
            
            # LICs/LITs - Usually 100% franked
            'AFI': {'franking_rate': 100, 'sector': 'Financials', 'reliability': 'high'},
            'ARG': {'franking_rate': 100, 'sector': 'Financials', 'reliability': 'high'},
            'MLT': {'franking_rate': 100, 'sector': 'Financials', 'reliability': 'high'},
            'WAM': {'franking_rate': 100, 'sector': 'Financials', 'reliability': 'high'},
            'WAX': {'franking_rate': 100, 'sector': 'Financials', 'reliability': 'high'},
            
            # Additional stocks with proper sector classifications
            'HLI': {'franking_rate': 100, 'sector': 'Financials', 'reliability': 'high'},  # Helia Group - Mortgage Insurance
            'YMAX': {'franking_rate': 70, 'sector': 'Financials', 'reliability': 'medium'},  # BetaShares YMAX ETF - Yield Maximizer
            'LNW': {'franking_rate': 0, 'sector': 'Consumer Discretionary', 'reliability': 'high'},  # Light & Wonder - Gaming/Entertainment
            'DTR': {'franking_rate': 0, 'sector': 'Materials', 'reliability': 'medium'},  # Dateline Resources Ltd - Mining/Mineral Exploration
            'SDR': {'franking_rate': 0, 'sector': 'Technology', 'reliability': 'high'},  # SiteMinder Ltd - SaaS/Technology
            'PME': {'franking_rate': 100, 'sector': 'Healthcare', 'reliability': 'high'},  # Pro Medicus - Medical Imaging Software
            
            # ETFs - Variable
            'VAS': {'franking_rate': 70, 'sector': 'Financials', 'reliability': 'medium'},
            'VTS': {'franking_rate': 0, 'sector': 'Financials', 'reliability': 'high'},
            'VEU': {'franking_rate': 0, 'sector': 'Financials', 'reliability': 'high'},
            'IOZ': {'franking_rate': 70, 'sector': 'Financials', 'reliability': 'medium'},
            
            # Other major stocks
            'MQG': {'franking_rate': 100, 'sector': 'Financials', 'reliability': 'high'},
            'REA': {'franking_rate': 100, 'sector': 'Technology', 'reliability': 'high'},
            'CAR': {'franking_rate': 100, 'sector': 'Technology', 'reliability': 'medium'},
            'SEK': {'franking_rate': 100, 'sector': 'Technology', 'reliability': 'medium'},
        }
    
    def get_franking_rate(self, stock: str) -> float:
        """Get franking rate for a stock (0-100)"""
        return self.franking_data.get(stock, {}).get('franking_rate', 50.0)  # Default 50%
            ...
            if announcements:
                ...
                if franking_info:
                    ...
                    self.franking_data[stock] = {
                        ...
                        'sector': franking_info.get('sector', self.franking_data.get(stock, {}).get('sector', 'Unknown')),
        ...
        try:
            ...
            for announcement in announcements:
                if isinstance(announcement, dict):
                    ...
                    if any(keyword in title for keyword in ['dividend', 'distribution', 'franking', 'interim', 'final']):
    ...
    def _parse_franking_from_announcements(self, announcements: List[Dict]) -> Optional[Dict]:
        ...
        for announcement in announcements:
            ...
            if any(keyword in combined_text for keyword in ['dividend', 'franking', 'franked', 'distribution']):
                ...
                for pattern in franking_patterns:
                    ...
                    if match:
                        ...
                        print(f"Found franking info: {franking_rate}% from announcement: {title[:50]}...")
    ...
    def get_sector_average_franking(self, sector: str) -> float:
        ...
        sector_stocks = [data for data in self.franking_data.values() if data['sector'] == sector]
        if sector_stocks:
    ...
    def bulk_update_franking_from_api(self, stocks: List[str], api_key: str = None) -> Dict[str, Dict]:
        ...
        for stock in stocks:
            ...
            if result:
                ...
                print(f"📋 Static data: {current_info['franking_rate']}% franked ({current_info['reliability']})")
...
class FrankingTaxCalculator:
    ...
    def calculate_franking_benefit(self, positions: Dict, taxable_income: float = 85000, 
        ...
        for stock, position in positions.items():
            ...
            if market_value == 0:
                market_value = getattr(position, 'quantity', 0) * getattr(position, 'current_price', 0)
            ...
            franking_credit = (annual_dividend * franking_rate * self.company_tax_rate) / (1 - self.company_tax_rate)
            
            total_dividend_income += annual_dividend
            total_franking_credits += franking_credit
            
            # Calculate effective yield including franking
            effective_yield = (annual_dividend + franking_credit) / market_value * 100 if market_value > 0 else 0
        ...
        tax_without_franking = self.calculate_tax_on_income(taxable_income + total_dividend_income)
        ...
        return {
            ...
            'franking_efficiency': (total_franking_credits / total_dividend_income * 100) if total_dividend_income > 0 else 0,
            'stock_details': stock_details,
            'calculation_date': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        }
    
    def get_optimization_suggestions(self, positions: Dict, taxable_income: float = 85000) -> List[Dict]:
        ...
        for stock_detail in current_franking['stock_details']:
            if stock_detail['franking_rate'] < 50 and stock_detail['market_value'] > 1000:
                # Suggest high-franking alternatives in same sector
                sector = stock_detail['sector']
                high_franking_stocks = [
                    stock for stock, data in self.franking_db.franking_data.items()
                ...
                if high_franking_stocks:
                    suggestions.append({
                        ...
                        'potential_benefit': stock_detail['market_value'] * 0.04 * 0.3,  # Rough estimate
                        'priority': 'high' if stock_detail['market_value'] > 5000 else 'medium'
                    })
        
        # Suggest increasing high-franking positions
        high_franking_stocks = [
            detail for detail in current_franking['stock_details']
            if detail['franking_rate'] >= 80
        ]
        
        if high_franking_stocks:
            best_franking_stock = max(high_franking_stocks, key=lambda x: x['franking_rate'])
            suggestions.append({
                ...
                'potential_benefit': 5000 * 0.04 * (best_franking_stock['franking_rate'] / 100) * 0.3,
                'priority': 'medium'
            })
...
if __name__ == "__main__":
    ...
    # Test data
    test_positions = {
        'CBA': type('Position', (), {'market_value': 10000, 'quantity': 50, 'current_price': 200})(),
        'CSL': type('Position', (), {'market_value': 5000, 'quantity': 20, 'current_price': 250})(),
        'BHP': type('Position', (), {'market_value': 8000, 'quantity': 200, 'current_price': 40})()
    }
