...
class OHLCCollector:
    ...
    def setup_ohlc_tables(self) -> bool:
        ...
        try:
            ...
            cursor.execute(
                "INSERT OR IGNORE INTO ohlc_config (key, value) VALUES (?, ?)",
                ("enabled", "true"),
            )
            cursor.execute(
                "INSERT OR IGNORE INTO ohlc_config (key, value) VALUES (?, ?)",
    ...
    def fetch_ohlc_eodhd(
        self, stock: str, api_key: str, date: str = None
    ) -> Optional[OHLCData]:
        ...
        try:
            ...
            params = {"api_token": api_key, "fmt": "json", "from": date, "to": date}
    ...
    def fetch_historical_ohlc_eodhd(
        self, stock: str, api_key: str, from_date: str, to_date: str = None
    ) -> List[OHLCData]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"✅ Fetched {len(ohlc_list)} historical OHLC records for {stock}"
                )
                return ohlc_list

            logger.warning(
                f"⚠️  No historical EODHD data for {stock} from {from_date} to {to_date}"
            )
            return []
    ...
    def fetch_historical_ohlc_yfinance(
        self, stock: str, from_date: str, to_date: str = None, period: str = None
    ) -> List[OHLCData]:
        ...
        try:
            ...
            logger.info(
                f"✅ Fetched {len(ohlc_list)} historical OHLC records for {stock} from Yahoo Finance"
            )
            return ohlc_list
        ...
        try:
            ...
            """,
                (
                    optimal.stock,
                    optimal.date,
                    optimal.actual_close,
                    optimal.optimal_buy_price,
                    optimal.optimal_sell_price,
                    optimal.vwap,
                    optimal.price_range_pct,
                    optimal.missed_opportunity_buy,
                    optimal.missed_opportunity_sell,
                ),
            )
    ...
    def collect_stock_ohlc(
        ...
        if ohlc_data:
            ...
            if self.store_ohlc_data(ohlc_data):
                ...
                logger.info(
                    f"📊 {stock}: Open=${ohlc_data.open_price:.3f}, "
    ...
    def collect_portfolio_ohlc(
        ...
        logger.info(
            f"🔄 Starting OHLC collection for {len(stocks)} stocks (Priority: {'Yahoo Finance' if prefer_yfinance else 'EODHD'})"
        )

        for stock in stocks:
            results[stock] = self.collect_stock_ohlc(
                stock, api_key, date, prefer_yfinance
            )
        ...
        if prefer_yfinance:
            logger.info(
                f"📈 OHLC collection complete: {successful}/{len(stocks)} successful"
            )
            logger.info(
                f"💡 EODHD API calls saved: {len(stocks) - eodhd_calls_used}/20 (Yahoo Finance primary)"
            )
        else:
            logger.info(
                f"📈 OHLC collection complete: {successful}/{len(stocks)} successful (EODHD primary)"
            )
    ...
    def get_optimal_pricing_analysis(
        self, stock: str, days: int = 30
    ) -> List[OptimalPricing]:
            ...
            for row in cursor.fetchall():
                results.append(
                    OptimalPricing(
                        ...
                        date=row[1],
                        actual_close=row[2],
                        optimal_buy_price=row[3],
                        optimal_sell_price=row[4],
                        vwap=row[5],
                        price_range_pct=row[6],
                        missed_opportunity_buy=row[7],
                        missed_opportunity_sell=row[8],
                    )
                )
    ...
    def collect_historical_data(
        ...
        if prefer_yfinance:
            ...
            ohlc_list = self.fetch_historical_ohlc_yfinance(
                stock, from_date, to_date, period=period
            )

            # Fallback to EODHD if Yahoo Finance fails
            if not ohlc_list and api_key != "demo":
                logger.info(f"⚠️  Yahoo Finance failed for {stock}, trying EODHD...")
                ohlc_list = self.fetch_historical_ohlc_eodhd(
                    stock, api_key, from_date, to_date
                )
        ...
        logger.info(
            f"✅ Stored {stored_count}/{len(ohlc_list)} historical records for {stock}"
        )
        return stored_count

    def collect_portfolio_historical_data(
        self, days: int = 30, api_key: str = "demo", prefer_yfinance: bool = True
    ) -> Dict[str, int]:
        ...
        logger.info(
            f"🔄 Collecting {days} days of historical data for {len(stocks)} stocks"
        )
        ...
        for stock in stocks:
            try:
                count = self.collect_historical_data(
                    stock, days, api_key, prefer_yfinance
                )
                results[stock] = count
                total_records += count
            except Exception as e:
        ...
        logger.info(
            f"🎉 Historical collection complete: {len(successful_stocks)}/{len(stocks)} stocks, {total_records} total records"
        )
    ...
    def fill_missing_data(
        self, api_key: str = "demo", prefer_yfinance: bool = True
    ) -> Dict[str, int]:
        ...
        for stock in stocks:
            ...
            if missing_dates:
                ...
                for date in missing_dates:
                    try:
                        success = self.collect_stock_ohlc(
                            stock, api_key, date, prefer_yfinance
                        )
                        if success:
                            filled_count += 1
                ...
                if filled_count > 0:
                    logger.info(
                        f"✅ Filled {filled_count}/{len(missing_dates)} missing dates for {stock}"
                    )
            else:
                results[stock] = 0
    ...
    def cleanup_old_data(self, retention_days: int = 90):
        ...
        try:
            ...
            cursor.execute(
                "DELETE FROM optimal_pricing_analysis WHERE date < ?", (cutoff_date,)
            )
            ...
            logger.info(
                f"🧹 Cleaned up {ohlc_deleted} OHLC records and {analysis_deleted} analysis records older than {retention_days} days"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OHLC Data Collector for ASX Portfolio Tracker"
    )
    parser.add_argument("--stock", "-s", help="Collect OHLC for specific stock")
    parser.add_argument(
        "--all", "-a", action="store_true", help="Collect OHLC for all portfolio stocks"
    )
    ...
    parser.add_argument(
        "--disable", action="store_true", help="Disable OHLC collection"
    )
    parser.add_argument(
        "--status", action="store_true", help="Show OHLC collection status"
    )
    parser.add_argument(
        "--prefer-eodhd",
        action="store_true",
        help="Use EODHD as primary (default: Yahoo Finance primary)",
    )
    parser.add_argument(
        "--save-api-calls",
        action="store_true",
        help="Use Yahoo Finance only (save all EODHD calls)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="DAYS",
        help="Collect historical OHLC data for N days",
    )
    parser.add_argument(
        "--fill-gaps", action="store_true", help="Fill missing OHLC data gaps"
    )
    parser.add_argument(
        "--cleanup",
        type=int,
        metavar="DAYS",
        help="Clean up OHLC data older than N days",
    )
    ...
    if args.enable:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE ohlc_config SET value = ? WHERE key = ?", ("true", "enabled")
        )
        conn.commit()
        conn.close()
    ...
    if args.disable:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE ohlc_config SET value = ? WHERE key = ?", ("false", "enabled")
        )
        conn.commit()
        conn.close()
    ...
    if args.stock:
        if args.save_api_calls:
            ...
            success = collector.collect_stock_ohlc(
                args.stock.upper(), "demo", args.date, prefer_yfinance=True
            )
        else:
            success = collector.collect_stock_ohlc(
                args.stock.upper(), args.api_key, args.date, prefer_yfinance
            )
        ...
        if stocks:
            ...
            if args.save_api_calls:
                # Yahoo Finance only mode
                results = collector.collect_portfolio_ohlc(
                    stocks, "demo", args.date, prefer_yfinance=True
                )
            else:
                results = collector.collect_portfolio_ohlc(
                    stocks, args.api_key, args.date, prefer_yfinance
                )
        ...
        results = collector.fill_missing_data(
            args.api_key, prefer_yfinance=prefer_yfinance
        )
