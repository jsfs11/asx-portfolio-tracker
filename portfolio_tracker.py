...
class ASXPortfolioTracker:
    ...
    def get_current_price_eodhd(self, stock_symbol: str, api_key: str = "demo", force: bool = False) -> Optional[float]:
        """Get current price from EODHD API"""
        if not force and self.api_calls_today >= 20:
            print(f"⚠️  API limit reached for today ({self.api_calls_today}/20). Using stored prices.")
            return None
    ...
    def get_eod_price_eodhd(self, stock_symbol: str, api_key: str = "demo") -> Optional[float]:
    ...
    def update_current_prices(self, api_key: str = "demo", use_api: bool = False, force: bool = False):
        ...
        for stock in positions.keys():
            ...
            if price:
                ...
                positions[stock].unrealized_pnl = positions[stock].market_value - cost_basis
