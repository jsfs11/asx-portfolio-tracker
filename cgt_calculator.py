...
class CGTCalculator:
        ...
        for transaction in transactions:
            ...
            if action == "buy":
                ...
                try:
                    ...
                    print(
                        f"Warning: Could not process sale of {quantity} {stock} on {date}: {e}"
                    )
                    continue
    ...
    def _calculate_cgt_for_historical_sale(
        self,
        cursor,
        stock: str,
        sale_date: str,
        ...
        if remaining_to_sell > 0:
            raise ValueError(
                f"Insufficient shares available for {stock}. Need {quantity}, but only {quantity - remaining_to_sell} available before {sale_date}."
            )

        return cgt_events

    def calculate_cgt_for_sale(
        self,
        stock: str,
        sale_date: str,
        quantity: int,
        ...
        if remaining_to_sell > 0:
            conn.close()
            raise ValueError(
                f"Insufficient shares available. Need {quantity}, but only {quantity - remaining_to_sell} available."
            )
    ...
    def calculate_annual_cgt(self, tax_year: str) -> CGTSummary:
        ...
        return CGTSummary(
            ...
            carried_forward_losses=carried_losses
            - (used_carried_losses if current_year_net > 0 else 0),
        )
    ...
    def get_unrealised_gains(self, current_prices: Dict[str, float]) -> List[Dict]:
        ...
        for stock, acq_date, remaining_qty, cost_base, original_qty in parcels:
            if stock in current_prices:
                ...
                holding_days = (
                    datetime.now() - datetime.strptime(acq_date, "%Y-%m-%d")
                ).days
                ...
                unrealised.append(
                    {
                        ...
                        "after_discount": (
                            unrealised_gain * 0.5
                            if discount_eligible and unrealised_gain > 0
                            else unrealised_gain
                        ),
                    }
                )
    ...
    def optimize_cgt_sale(
        self, stock: str, target_gain: float, current_price: float
    ) -> List[Dict]:
        ...
        for parcel in parcels:
            ...
            effective_gain_per_share = (
                gain_per_share * 0.5
                if discount_eligible and gain_per_share > 0
                else gain_per_share
            )
            ...
            suggestions.append(
                {
                    ...
                    "shares_for_target": (
                        int(target_gain / effective_gain_per_share)
                        if effective_gain_per_share > 0
                        else 0
                    ),
                }
            )
...
def generate_cgt_report(calculator: CGTCalculator, tax_year: str):
    ...
    if events:
        print(f"\nDETAILED CGT EVENTS:")
        print(
            f"{'Date':<12} {'Stock':<6} {'Qty':<6} {'Gain/Loss':<12} {'Discounted':<12} {'Held':<10}"
        )
        print("-" * 70)
...
if __name__ == "__main__":
    ...
    try:
        ...
        for event in events:
            ...
            print(
                f"  Held for: {(datetime.strptime(event.sale_date, '%Y-%m-%d') - datetime.strptime(event.acquisition_date, '%Y-%m-%d')).days} days"
            )
